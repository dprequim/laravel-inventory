@extends('layouts.app', ['page' => 'APR Stock-in', 'pageSlug' => 'apr_in-create', 'section' => 'transactions'])
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">Agency Procurement Request Stock-out</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('apr_in.index') }}" class="btn btn-sm btn-primary">Back to List</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('apr_in.store') }}" autocomplete="off">
                            @csrf
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('stock_in') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-stock_in">Stock-in</label>
                                    <input type="text" name="stock_in" id="input-stock_in" class="form-control form-control-alternative{{ $errors->has('stock_in') ? ' is-invalid' : '' }}" value="{{ old('stock_in') }}" required>
                                    @include('alerts.feedback', ['field' => 'stock_in'])
                                </div>
