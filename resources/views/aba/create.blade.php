@extends('layouts.app', ['page' => 'New ABA', 'pageSlug' => 'ABA-create', 'section' => 'transactions'])
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">ABSTRACT OF BIDS AND AWARDS</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('aba.index') }}" class="btn btn-sm btn-primary">Back to List</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('aba.store') }}" autocomplete="off">
                            @csrf
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('parriculars') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-parriculars">PARRICULARS</label>
                                    <input type="text" name="parriculars" id="input-parriculars" class="form-control form-control-alternative{{ $errors->has('parriculars') ? ' is-invalid' : '' }}" value="{{ old('parriculars') }}" required>
                                    @include('alerts.feedback', ['field' => 'parriculars'])
                                </div>
                                <div class="form-group{{ $errors->has('budget_cost') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-budget_cost">BUDGET COST</label>
                                    <input type="text" name="budget_cost" id="input-budget_cost" class="form-control form-control-alternative{{ $errors->has('budget_cost') ? ' is-invalid' : '' }}" value="{{ old('budget_cost') }}" required>
